...
cli
    ...
    const { createServer } = await Promise.resolve().then(function () { return require('./chunks/lib-3fb7872b.js'); });
...
cli.command('build [root]', 'build for production').action(async (root, options) => {
    const { build } = await Promise.resolve().then(function () { return require('./chunks/lib-0b6fdb51.js'); });
    const inlineConfig = createInlineConfig(root, options);
    if (options.entry) {
        process.env.ELECTRON_ENTRY = options.entry;
    }
...
cli
    ...
    .action(async (root, options) => {
    const { preview } = await Promise.resolve().then(function () { return require('./chunks/lib-654921af.js'); });
    const inlineConfig = createInlineConfig(root, options);
    if (options.noSandbox) {
        process.env.NO_SANDBOX = '1';
    }
